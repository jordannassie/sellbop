  ...
  if (notFound || !store) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-50 px-4">
        ...
        <Link href="/marketplace" className="text-sm text-neutral-600 hover:text-black transition-colors">
          ← Explore Marketplace
        </Link>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col items-center text-center py-12 border-b border-neutral-100">
          <div className="w-24 h-24 rounded-full overflow-hidden bg-neutral-200 mb-5 flex items-center justify-center shadow-sm">
            {store.avatar_url ? (
          ...
          {store.headline && (
            <p className="text-base text-neutral-500 mb-2">{store.headline}</p>
          )}
          {store.bio && (
            <p className="text-sm text-neutral-400 max-w-md leading-relaxed">{store.bio}</p>
          )}
        </div>
        ...
        <div className="py-10">
          {products.length === 0 ? (
            <div className="text-center py-20">
              <Package size={36} className="mx-auto mb-3 text-neutral-200" />
              <p className="text-neutral-400 text-sm">No products published yet.</p>
            </div>
          ) : (
            <>
