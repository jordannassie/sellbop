...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, label, error, hint, id, ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <textarea ref={ref} id={inputId} rows={4} className={cn('w-full px-3 py-2 text-sm rounded-lg border border-neutral-200 bg-white text-neutral-900 placeholder:text-neutral-400 resize-none focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent', error && 'border-red-400', className)} {...props} />
