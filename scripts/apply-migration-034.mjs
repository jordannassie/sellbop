#!/usr/bin/env node
/** Apply migration 034_oauth_access_mode.sql. Usage: npm run db:apply-034 */
...
const migrationFile = '034_oauth_access_mode.sql'
...
if (!databaseUrl) {
  console.error('Missing valid DATABASE_URL.')
  console.error('Or paste APPLY-034-IN-SUPABASE.sql in Supabase SQL Editor.')
  process.exit(1)
}
...
async function main() {
  ...
  const { rows } = await client.query(`
    ...
    WHERE table_schema = 'public' AND table_name = 'oauth_authorization_codes' AND column_name = 'access_mode'
  `)
  console.log('oauth_authorization_codes.access_mode:', rows.length ? 'EXISTS' : 'MISSING')
  await client.end()
  console.log('Done.')
}
