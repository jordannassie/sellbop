#!/usr/bin/env node
/**
 * Apply migration 025_partner_badge.sql
 *
 * Usage:
 *   npm run db:apply-025
 */
...
const migrationFile = '025_partner_badge.sql'
...
if (!databaseUrl) {
  ...
  console.error('Set it in the environment or in .env.local, then run: npm run db:apply-025')
  process.exit(1)
}
...
async function main() {
  ...
  const { rows } = await client.query(`
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name IN ('is_partner', 'show_partner_badge')
