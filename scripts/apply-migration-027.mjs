#!/usr/bin/env node
/**
 * Apply migration 027_partner_applications.sql
 *
 * Usage:
 *   npm run db:apply-027
 */
...
const migrationFile = '027_partner_applications.sql'
...
if (!databaseUrl) {
  ...
  console.error('Set it in the environment or in .env.local, then run: npm run db:apply-027')
  process.exit(1)
}
...
async function main() {
  ...
  `)
  console.log('partner_applications rows:', rows[0])
  await client.end()
  console.log('Done.')
}
