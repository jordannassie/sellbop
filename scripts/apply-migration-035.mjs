#!/usr/bin/env node
/** Apply migration 035_value_video_url.sql. Usage: npm run db:apply-035 */
...
const migrationFile = '035_value_video_url.sql'
...
if (!databaseUrl) {
  ...
  console.error('Or paste APPLY-035-IN-SUPABASE.sql in Supabase SQL Editor.')
  process.exit(1)
}
...
async function main() {
  ...
  const { rows } = await client.query(`
    ...
    WHERE table_schema = 'public' AND table_name = 'stores' AND column_name = 'value_video_url'
  `)
  console.log('stores.value_video_url:', rows.length ? 'EXISTS' : 'MISSING')
  await client.end()
  console.log('Done.')
}
