...
async function createConnection(admin, userId, { accessMode, storeId, label }) {
  ...
  const { data, error } = await admin.from('agent_connections').insert({
    user_id: userId,
    store_id: storeId,
...
async function main() {
  ...
  try {
    ...
    if (juiceShop) {
      const jordanUserId = juiceShop.owner_user_id
    ...
    if (createdConnectionIds.length) {
      console.log(`  · revoked ${createdConnectionIds.length} test connection(s)`)
    }
  }
