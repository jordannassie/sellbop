#!/usr/bin/env node
/**
 * Apply migration 033_creative_generation.sql to production.
 * Usage: npm run db:apply-033
 */
...
const migrationFile = '033_creative_generation.sql'
...
if (!databaseUrl) {
  ...
  console.error('Set it in the environment or in .env.local, then run: npm run db:apply-033')
  console.error('\nOr paste supabase/migrations/033_creative_generation.sql in Supabase SQL Editor.')
  process.exit(1)
}
...
async function main() {
  ...
  const { rows: tableRow } = await client.query(`
    ...
    WHERE table_schema = 'public' AND table_name = 'creative_generation_usage'
  `)
  console.log('creative_generation_usage:', tableRow.length ? 'EXISTS' : 'MISSING')
  await client.end()
  console.log('Done.')
}
