#!/usr/bin/env node
/** Apply migration 036_product_ideas.sql. Usage: npm run db:apply-036 */
...
const migrationFile = '036_product_ideas.sql'
...
if (!databaseUrl) {
  ...
  console.error('Or paste APPLY-036-IN-SUPABASE.sql in Supabase SQL Editor.')
  process.exit(1)
}
...
async function main() {
  ...
  const { rows } = await client.query(`
    ...
    WHERE table_schema = 'public' AND table_name = 'product_ideas'
  `)
  console.log('product_ideas:', rows.length ? 'EXISTS' : 'MISSING')
  await client.end()
  console.log('Done.')
}
