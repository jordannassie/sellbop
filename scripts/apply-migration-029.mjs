#!/usr/bin/env node
/**
 * Apply migration 029_multi_store_foundation.sql
 *
 * Usage:
...
const migrationFile = '029_multi_store_foundation.sql'
...
if (!databaseUrl) {
  ...
  console.error('Set it in the environment or in .env.local, then run: npm run db:apply-029')
  process.exit(1)
}
...
async function main() {
  ...
  `)
  console.log('store_members rows:', rows[0])
  await client.end()
  console.log('Done.')
}
